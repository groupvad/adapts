...
def evaluate(args, category, model, visualizer, global_step=0):
    ...
    with torch.no_grad():
        ...
        aupro_seg, ap_seg, auc_seg, auc_detect_seg, f1_detect, f1_seg = (
            seg_AUPRO.compute(),
            seg_AP.compute(),
            seg_AUROC.compute(),
            seg_detect_AUROC.compute(),
            seg_detect_F1.compute(),
            seg_F1.compute()
        )
        ...
        visualizer.add_scalar("DeSTSeg_detect_F1", f1_detect, global_step)
        visualizer.add_scalar("DeSTSeg_F1", f1_seg, global_step)

        wandb.log(
            {
                "DeST_IAP": iap_de_st,
                "DeST_IAP90": iap90_de_st,
                "DeST_AUPRO": aupro_de_st,
                "DeST_AP": ap_de_st,
                "DeST_AUC": auc_de_st,
                "DeST_detect_AUC": auc_detect_de_st,
                "DeSTSeg_IAP": iap_seg,
                "DeSTSeg_IAP90": iap90_seg,
                "DeSTSeg_AUPRO": aupro_seg,
                "DeSTSeg_AP": ap_seg,
                "DeSTSeg_AUC": auc_seg,
                "DeSTSeg_detect_AUC": auc_detect_seg,
                "DeSTSeg_detect_F1": f1_detect,
                "DeSTSeg_F1": f1_seg,
        ...
        print("F1:", round(float(f1_seg), 4))
        print("Detect F1:", round(float(f1_detect), 4))
        print()
